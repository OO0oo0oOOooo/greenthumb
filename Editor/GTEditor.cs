...
public partial class GTEditor
{
    ...
    private void SelectedPrefabGUI()
    {
        ...
        if(_showPaletteItems = EditorGUILayout.Foldout(_showPaletteItems, "Palette Item"))
        {
            ...
            // // Enum
            // _selectedPaletteItem.ActivePrefabScaleMode = EditorGUILayout.EnumPopup(_selectedPaletteItem.ActivePrefabScaleMode);

            // // Prefab
            // _selectedPaletteItem.Prefab = EditorGUILayout.ObjectField(_selectedPaletteItem.Prefab, typeof(GameObject), false) as GameObject;
            // GUILayout.Space(10);
        
            // // Draw the appropriate fields based on the enum selection
            // switch (_selectedPaletteItem.ActivePrefabScaleMode)
            // {
            //     case PrefabScaleMode.Fixed:
            //         using ( new GUILayout.HorizontalScope() )
            //         {
            //             GUILayout.Label("Width");
            //             EditorGUILayout.Slider(_selectedPaletteItem.Width, 0, 1);
            //         }
            //         using ( new GUILayout.HorizontalScope() )
            //         {
            //             GUILayout.Label("Height");
            //             EditorGUILayout.Slider(_selectedPaletteItem.Height, 0, 1);
            //         }
            //         break;

            //     case PrefabScaleMode.Weighted:
            //         // EditorGUILayout.PropertyField(_selectedPaletteItem.ScaleWeighted);
            //         // EditorGUILayout.PropertyField(_selectedPaletteItem.Weights);
            //         break;
            // }
