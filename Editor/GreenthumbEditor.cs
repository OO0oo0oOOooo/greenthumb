...
public partial class GreenthumbEditor : EditorWindow
{
    ...
    private void LoadData()
    {
        ...
        if(_obj != null)
        {
            LoadChunksFromRenderer(_obj.GetComponent<GTGrassRenderer>().Chunks.ToArray());
        }
    }
    ...
    private void Place(int tab, RaycastHit hit)
    {
        ...
        {
            case (int)Tabs.Tree:
                InstatiatePrefab(hit, _selectedPaletteItem.Prefab, _obj, _selectedPaletteItem.Scale, _brushSettings.BrushNormalLimit, _brushSettings.BrushNormalWeight, _layer);
                break;
    ...
    private void InstatiatePrefab(RaycastHit hit, GameObject prefab, GameObject parent, Vector3 scale, float normalLimit, float normalWeight, int layer)
    {
        ...
        GameObject newObject = _obj != null ?
    ...
    private void RemovePrefab(RaycastHit hit, float size)
    {
        ...
        foreach (var hitCollider in hitColliders)
        {
            ...
            if(_obj != null)
            {
                if(hitCollider.transform.root.gameObject != _obj)
                    continue;
                
                obj = GreenthumbUtils.FindRootByParent(hitCollider.gameObject, _obj);
            }
            else
            {
